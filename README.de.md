...
- **Nichts von Hand notieren.** In Claude Code laufen **9 Hooks** zu festen Zeitpunkten: Sitzungsstart, vor Dateiänderungen, nach `git commit`, nach einem freigegebenen Plan oder einer beantworteten Frage, wenn Claude aufhört, vor dem Kürzen des Kontexts, wenn du „remember this“ sagst (in 5 Sprachen, Deutsch nicht darunter), vor einem riskanten Befehl, der eine von dir bestätigte Lektion wiederholen würde, und wenn eine Codex-Sitzung mit MeMesh-Plugin startet oder fortgesetzt wird.
- **Ein Gedächtnis für alle Tools.** Was du heute in Claude Code speicherst, steht morgen auch Codex oder Cursor zur Verfügung.
- **Agenten können sich Nachrichten hinterlassen.** Der dauerhafte lokale Posteingang übersteht Neustarts; eine exakt adressierte aktive Codex-Plugin-Sitzung kann die begrenzte Nachricht zusätzlich über ihre native Queue erhalten.
...
- Ein laufender Agent kann mit `work_package` einen Kalender-Digest oder begrenzte sichtbare Züge aus dem neuesten Claude-Code-Transkript unter dem einzigen passenden MCP-Workspace-Root des Clients vorbereiten. Die Einreichung bewahrt redigierte Quellzüge auf und stellt nur einen Vorschlag zur menschlichen Prüfung bereit; MeMesh kontaktiert keinen Provider.
