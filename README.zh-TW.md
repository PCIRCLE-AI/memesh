...
請已在執行的 agent 準備一個 `work_package`。它可以依日曆選取一組摘要；若 MCP host 提供唯一且符合專案的 workspace root，而且該 workspace 有近期 Claude Code 對話記錄，也可以從最新的 Claude Code session 準備有界限的可見輪次。接著，agent 提交一份有界結果或選擇延後。提交只會暫存提案：請在儀表板比較提議的記憶與保留的遮蔽來源輪次，再由你接受或拒絕。儀表板不能啟動或喚醒 agent。
...
MeMesh 的回憶與擷取保持本機且可預測：SQLite FTS5 搜尋、明確的記憶工具，以及規則式 hooks。需要摘要，或同一個 MCP workspace 的近期 Claude Code session 裡仍有值得保留的知識時，已在執行的 agent 可以使用 `work_package`。Transcript 模式要求 client 提供唯一且符合專案的 MCP workspace root；root 缺少或有歧義時會直接拒絕。支援互動提示的 host 會顯示簡短選項：**派遣 agent 任務**、**稍後**、**本次工作階段不要再建議**。最後一個選項只會在本次工作階段停止提示，不會建立持久的取消建議設定。結果一律等待人類審核；沒有背景模型、供應商設定、排程挖掘或由儀表板派遣任務。
...
| 工具 | 做什麼 |
|------|--------|
| `work_package` | 準備一個有界限且不受信任的套件：`digest` 依日曆選取群組；`transcript` 從 client 唯一且符合專案的 MCP workspace root 所對應的最新 Claude Code session 選取可見輪次。agent 只可提交一個嚴格驗證的結果或延後；提交只會暫存為等待人類審核的提案，並保留有界限且已遮蔽的來源輪次供比較。不會暴露隱藏推理、工具流量、原始逐字稿或逐字稿檔案路徑；可辨識的憑證格式會被遮蔽。不會使用或暴露 LLM、嵌入或向量資料。`work_package` 的 MCP 合約不提供接受／拒絕操作；本機 Dashboard 與 CLI 審核介面提供這些操作，但不會以密碼學方式驗證操作者為人類。雜湊只用來辨識新鮮度與 workspace 範圍，而非驗證身份。 |
