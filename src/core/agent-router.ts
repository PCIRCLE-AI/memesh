...
/**
 * Dedicated host adapters implement this interface from core. They do not
 * import CLI commands, transports, or router internals. Authentication proves
 * that the named host session is active; it grants no tools or permissions.
 */
...
export type AgentRouterRequest =
  | AgentRouterRegisterRequest
  | AgentRouterDiscoverRequest
  | AgentRouterNotifyRequest
  | AgentRouterHeartbeatRequest
  | AgentRouterDisconnectRequest
  | AgentRouterHostAcceptRequest
  | AgentRouterHostRejectRequest;
...
export function isLegacyAgentRouterVersionMismatchResponse(value: unknown): boolean {
  if (!isPlainObject(value) || value.version !== 1 || value.request_id !== '' || value.ok !== false) return false;
...
export class AgentRouter {
  ...
  private readonly selectionCards = new Map<string, Omit<AgentSelectionCard, 'active' | 'generation' | 'lease_expires_at_ms'>>();
  ...
  private readonly pendingExternal = new Map<string, {
    ...
    delivery_id: string;
    resolve: (result: AgentHostDispatchResult) => void;
  ...
  constructor(options: AgentRouterOptions) {
    ...
    for (const adapter of options.adapters) {
      ...
      if (this.adapters.has(kind)) throw new AgentRouterError('duplicate_adapter', `Duplicate adapter ${kind}.`);
      this.adapters.set(kind, adapter);
    }
  }

  async start(): Promise<void> {
    if (this.server) throw new AgentRouterError('already_started', 'The agent router is already started.');
    if (process.platform === 'win32') {
      throw new AgentRouterError(
        'unsupported_secure_host_runtime',
        ...
        try {
          ...
          if (!await removeOrphanedSocket(this.socket_path)) {
            throw new AgentRouterError(
              ...
              'A live or indeterminate router already owns the socket.',
            );
          }
        }
      }
  ...
  async stop(): Promise<void> {
    ...
    if (socketIdentity) unlinkSocketIfSame(this.socket_path, socketIdentity);
  }
  ...
  private async handleRequest(request: AgentRouterRequest, socket: net.Socket): Promise<AgentJsonObject> {
    switch (request.type) {
      case 'register':
  ...
  private async register(request: AgentRouterRegisterRequest, socket: net.Socket): Promise<AgentJsonObject> {
    const adapter = this.adapters.get(request.adapter_kind);
    if (!adapter) throw new AgentRouterError('adapter_unavailable', 'The requested host adapter is unavailable.');
    const registration: AgentHostRegistration = {
  ...
  private disconnectSocket(socket: net.Socket): void {
    ...
    for (const [connectionId, bound] of this.externalConnections) {
      ...
      for (const [attemptId, pending] of this.pendingExternal) {
        if (pending.connection.connection_id !== connectionId) continue;
  ...
  private handleHostOutcome(
    request: AgentRouterHostAcceptRequest | AgentRouterHostRejectRequest,
    socket: net.Socket,
  ): AgentJsonObject {
  ...
  private registerConnection(registration: AgentHostRegistration): ConnectionRow {
    return this.db.transaction(() => {
      ...
      if (!existingPrincipal) {
        ...
        `).get(registration.project) as { sequence: number }).sequence;
        this.db.prepare(`
      ...
      `).get(registration.project, registration.session_instance_id) as
        | { principal_id: string; adapter_kind: string; last_generation: number }
        | undefined;
      if (session && (
        session.principal_id !== registration.principal_id || session.adapter_kind !== registration.adapter_kind
      )) {
        throw new AgentRouterError(
          'session_identity_conflict',
          'The session instance is already bound to another principal or adapter.',
        );
      }
      if (!session) {
        this.db.prepare(`
        ...
        `).run(
          ...
          registration.session_instance_id,
      ...
      `).run(
        ...
        connection.router_instance_id,
        connection.lease_expires_at_ms,
      );
  ...
  private discover(request: AgentRouterDiscoverRequest): AgentJsonObject {
    const cards: AgentSelectionCard[] = [];
    for (const [connectionId, card] of this.selectionCards) {
      if (card.project !== request.project) continue;
      ...
      `).get(connectionId, request.project, this.router_instance_id, Date.now()) as ConnectionRow | undefined;
      ...
      if (cards.length >= request.limit) break;
    }
    return { cards: cards as unknown as AgentJsonValue };
  }

  private requireCurrentConnection(request: {
    project: string;
    session_instance_id: string;
    connection_id: string;
    generation: number;
    ...
    if (!row) throw new AgentRouterStaleGenerationError();
    return row;
  }

  private insertPresenceFact(
    connection: ConnectionRow,
    kind: 'connected' | 'heartbeat' | 'disconnected' | 'superseded',
  ...
  private async drainConnection(connection: ConnectionRow, hops: number): Promise<number> {
    if (hops > this.limits.max_hops) throw new AgentRouterError('hop_limit', 'Router hop limit exceeded.');
    const checkpoint = this.db.prepare(`
      SELECT activation_event_sequence FROM agent_principals
      WHERE project = ? AND principal_id = ?
  ...
  private async dispatchDeliveryOnce(
    deliveryId: string,
    project: string,
    hops: number,
    preferred?: ConnectionRow,
    ...
    if (!externalSocket) return false;
  ...
  private loadDelivery(deliveryId: string, project: string): DeliveryRow {
    ...
    `).get(validateField('delivery_id', deliveryId), validateField('project', project)) as
      | DeliveryRow
      | undefined;
    if (!row) throw new AgentRouterError('delivery_not_found', 'The delivery does not exist in this project.');
    parseTargetKind(row.target_kind);
    return row;
  }

  private resolveConnection(delivery: DeliveryRow, preferred?: ConnectionRow): ConnectionRow | undefined {
    ...
    if (preferred && this.connectionMatchesDelivery(preferred, delivery, targetKind)) {
      try { return this.requireCurrentConnection(preferred); } catch { return undefined; }
    }
  ...
  private beginDispatchAttempt(
    delivery: DeliveryRow,
    connection: ConnectionRow,
  ): { attempt_id: string; attempt_number: number } {
    return this.db.transaction(() => {
      this.requireCurrentConnection(connection);
      ...
      `).get(delivery.delivery_id) as { next: number }).next;
  ...
  private finishAttempt(
    attemptId: string,
    result: 'adapter_returned' | 'adapter_rejected' | 'adapter_failed' | 'stale_generation',
  ...
  private writeError(socket: net.Socket, requestId: string, code: string, message: string): void {
  ...
  private writeResponse(socket: net.Socket, response: AgentRouterResponse): void {
    if (!socket.destroyed) socket.write(`${JSON.stringify(response)}\n`);
  }
}

export function createAgentRouterNotifier(socketPath: string): AgentMessagePostCommitNotifier {
...
export async function sendAgentRouterRequest(
  socketPath: string,
  ...
  return new Promise<AgentJsonObject>((resolve, reject) => {
    const socket = net.createConnection(validateSocketPath(socketPath));
    let pending = Buffer.alloc(0);
    let settled = false;
    const timer = setTimeout(() => finish(new AgentRouterProtocolError('timeout', 'Router request timed out.')),
      timeoutMs);

    const finish = (error?: Error, value?: AgentJsonObject) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      socket.destroy();
    ...
    socket.on('data', (chunk: Buffer) => {
      ...
      if (newline < 0) return;
      try {
        const response = JSON.parse(pending.subarray(0, newline).toString('utf8')) as AgentRouterResponse;
...
function isSelectionCard(value: unknown, project: string): value is AgentSelectionCard {
  ...
  return typeof value.session_id === 'string'
    ...
    && Number.isSafeInteger(value.lease_expires_at_ms) && (value.lease_expires_at_ms as number) >= 0;
}

function parseRequest(frame: Buffer, maxHops: number): AgentRouterRequest {
  let value: unknown;
  try { value = JSON.parse(frame.toString('utf8')); } catch {
    throw new AgentRouterProtocolError('invalid_json', 'Router frames must contain one JSON object.');
  }
  if (!isPlainObject(value)) throw new AgentRouterProtocolError('invalid_frame', 'Router frame must be an object.');
  ...
  switch (type) {
    ...
    case 'host_accept':
      ...
      return {
        ...
        connection_id: validateField('connection_id', value.connection_id),
        generation: validateInteger('generation', value.generation, 1, Number.MAX_SAFE_INTEGER),
        receipt: validateReceipt(value.receipt as AgentJsonObject),
        hops,
      };
    case 'host_reject':
      assertAllowedKeys(value, [
...
function assertAllowedKeys(value: Record<string, unknown>, allowed: readonly string[]): void {
  ...
  if (unexpected) {
    throw new AgentRouterProtocolError(
      'unexpected_field',
      `Router frame contains unsupported field ${unexpected}.`,
    );
  }
}

function validateField(label: string, value: unknown): string {
  if (typeof value !== 'string') throw new AgentRouterProtocolError('invalid_field', `${label} must be a string.`);
  const normalized = value.trim();
  if (!normalized || normalized.length > MAX_FIELD_LENGTH) {
    throw new AgentRouterProtocolError(
      'invalid_field',
      `${label} must contain 1-${MAX_FIELD_LENGTH} characters.`,
    );
  }
  return normalized;
}
...
function hostKind(adapterKind: string): AgentSelectionCard['host_kind'] {
  if (adapterKind === 'codex-cli-queue' || adapterKind === 'codex-app-server') return 'codex';
  if (adapterKind === 'claude-channel') return 'claude';
  if (adapterKind === 'acp') return 'gemini';
  return 'other';
}
...
async function removeOrphanedSocket(socketPath: string): Promise<boolean> {
  const initial = lstatIfPresent(socketPath);
  if (!initial) return true;
  assertSafeStaleSocketCandidate(initial);

  if (await probeSocket(socketPath) !== 'refused') return false;

  const afterProbe = lstatIfPresent(socketPath);
  if (!afterProbe || !sameSocketIdentity(initial, afterProbe)) return true;

  // A second refusal narrows the race between liveness probing and pathname
  // removal. The identity check below prevents deleting a replacement socket.
  if (await probeSocket(socketPath) !== 'refused') return false;
  const beforeUnlink = lstatIfPresent(socketPath);
  if (!beforeUnlink || !sameSocketIdentity(initial, beforeUnlink)) return true;
...
function probeSocket(socketPath: string): Promise<'connected' | 'refused' | 'missing' | 'indeterminate'> {
  return new Promise((resolve) => {
    ...
    const finish = (result: 'connected' | 'refused' | 'missing' | 'indeterminate') => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      socket.destroy();
...
function assertSafeStaleSocketCandidate(stat: fs.Stats): void {
  if (!stat.isSocket()) {
...
function readSocketIdentity(socketPath: string): SocketIdentity {
  ...
  if (!stat.isSocket()) throw new AgentRouterError('invalid_socket', 'Router did not create a Unix socket.');
  return { dev: stat.dev, ino: stat.ino };
}

function sameSocketIdentity(expected: SocketIdentity, actual: SocketIdentity): boolean {
  return expected.dev === actual.dev && expected.ino === actual.ino;
}

function unlinkSocketIfSame(socketPath: string, expected: SocketIdentity): void {
...
function normalizeLimits(limits: AgentRouterLimits = {}): Required<AgentRouterLimits> {
  return {
    max_frame_bytes: validateInteger(
      'max_frame_bytes',
      limits.max_frame_bytes ?? AGENT_ROUTER_MAX_FRAME_BYTES,
      256,
      AGENT_ROUTER_MAX_FRAME_BYTES,
    ),
    max_hops: validateInteger('max_hops', limits.max_hops ?? AGENT_ROUTER_MAX_HOPS, 1, 16),
    max_frames_per_window: validateInteger(
      'max_frames_per_window',
      limits.max_frames_per_window ?? DEFAULT_RATE_LIMIT,
      1,
      10_000,
    ),
    rate_window_ms: validateInteger(
      'rate_window_ms',
      limits.rate_window_ms ?? DEFAULT_RATE_WINDOW_MS,
      10,
      60 * 60_000,
    ),
    ...
    delivery_timeout_ms: validateInteger(
      'delivery_timeout_ms', limits.delivery_timeout_ms ?? 10_000, 100, 10 * 60_000,
    ),
  };
}

function validateReceipt(value: AgentJsonObject): AgentJsonObject {
  if (!isPlainObject(value)) throw new AgentRouterError('invalid_adapter_receipt', 'Adapter receipt must be an object.');
  let json: string;
...
function toRouterError(error: unknown): AgentRouterError {
  if (error instanceof AgentRouterError) return error;
