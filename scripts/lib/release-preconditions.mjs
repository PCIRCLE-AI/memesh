 ...
 * @param {number|null}   input.qaPreReleaseStatus   Exit code of `npm run qa:pre-release`, run by this script (its slowest step may reuse a receipt from an immediately preceding --dry-run of the same tree — see scripts/qa/pre-release.mjs CACHE_ENV_VAR); null if it could not run at all.
