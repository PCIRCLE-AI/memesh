...
- `update-entrypoint.ts` — the same notice for the other doors: one `[memesh update]` line on the MCP server's first successful tool result that has an answer for it, per process (appended as a second content item) and on CLI commands (stderr, once a day), suppressed when a hook announced it in the last ten minutes; when the cache is missing or stale it starts the hook's detached `memesh status` refresh (throttled to one per five minutes) and stays quiet until a check has completed
- `update-notice.ts` — the update-status resolver the hooks read (`UP_TO_DATE` / `UPGRADE_AVAILABLE` / `SNOOZED` / `JUST_UPGRADED` / `CHECK_FAILED` / `DISABLED`), escalating snooze, the post-upgrade receipt and the two-TTL refresh policy; a runtime leaf shared with the hooks via `scripts/hooks/_generated/update-notice.js`. Also owns `staleRunningProcessNotice`: a separate, unthrottled, stateless check the MCP server runs on every tool call (until it fires once) comparing the version it loaded at startup against a fresh re-read of `package.json`, since a plugin-marketplace upgrade replaces files on disk without signalling any already-running MCP connection — this is not the same code path as the registry-based notice above (no marker, no cache, no snooze), but the caller (`handlers.ts`'s `withFirstCallNotice`) does gate it on the same `updateCheck` config setting as the registry-based notice, so disabling update checks silences both.
...
- **Also on the same Stop**, `scripts/hooks/_stop-handoff.js` keeps the agent's own last message as the project's session handoff: one `session-handoff:<project>` entity, replaced on every Stop, redacted for known credential shapes, fenced code removed, at most 800 characters including a leading `…` (the end of the message is kept). It uses the payload's `last_assistant_message` and falls back to the newest assistant text in the last 256 KB of the transcript; the source is written into the `handoff-capture` outcome. A message under 80 characters after cleaning leaves the previous handoff in place. It costs no extra model turn, is gated on `MEMESH_AUTO_CAPTURE`, and no session-start injection lists it yet: it is kept out of the ranked block, the recent and project pools, the durable index, pre-edit recall, `memesh doctor`'s per-type trend, the project list and the patterns view (`recall` and the dashboard can still find it).
