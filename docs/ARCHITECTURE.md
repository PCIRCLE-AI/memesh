...
- `update-notice.ts` — the one update-status resolver (`UP_TO_DATE` / `UPGRADE_AVAILABLE` / `SNOOZED` / `JUST_UPGRADED` / `CHECK_FAILED` / `DISABLED`), escalating snooze, the post-upgrade receipt and the two-TTL refresh policy; a runtime leaf shared with the hooks via `scripts/hooks/_generated/update-notice.js`
