...
- `update-entrypoint.ts` — the same notice for the other doors: one `[memesh update]` line on the MCP server's first successful tool result per process (appended as a second content item) and on CLI commands (stderr, once a day), suppressed when a hook announced it in the last ten minutes
