...
**session-limit.ts** — The single policy for `sessionLimit` (#431): the documented range (`SESSION_LIMIT_MIN`/`SESSION_LIMIT_MAX`/`SESSION_LIMIT_DEFAULT`), the integer-range check, and `resolveSessionLimit(envRaw, configValue)` — env beats config beats the default, clamping a number above the max and falling through/back below it, reporting every source it had to adjust. `core/config.ts`, the CLI and the HTTP server import it directly; the SessionStart hook imports the generated copy (`_generated/session-limit.js`), since `core/config.ts` is not a leaf. A zero-import leaf, mirrored the same way as `briefing-level.ts` just above.
