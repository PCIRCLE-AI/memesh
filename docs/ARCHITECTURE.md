...
- **Also on the same Stop**, `scripts/hooks/_stop-handoff.js` keeps the agent's own last message as the project's session handoff: one `session-handoff:<project>` entity, replaced on every Stop, redacted, fenced code removed, at most 800 characters (the end of the message is kept). It uses the payload's `last_assistant_message` and falls back to the newest assistant text in the last 256 KB of the transcript; the source is written into the `handoff-capture` outcome. A message under 80 characters after cleaning leaves the previous handoff in place. It costs no extra model turn, is gated on `MEMESH_AUTO_CAPTURE`, and no injection path lists it yet: it is kept out of the ranked block, the durable index and pre-edit recall.
