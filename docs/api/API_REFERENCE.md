...
|---|---|---|---|---|---|---|---|
| `minimal` (default) | yes | yes | no | no | no | no | no |
| `standard` | yes | yes | yes | yes | no | no | no |
...
- Titles and snippets have credential-shaped secrets and user paths redacted. The heading and empty-state line use the project's readable label without redaction; the `N more` command prints a literal `"project:…"` placeholder rather than a real project name.
...
- `hooks` — one summary per hook, over its last 20 triggered outcome records
  ...
  either, and none of these hooks that `silent` applies to ever notifies.
- `types` — auto-capture entities per type, this week (`last7`) against the
  ...
  this week. The single per-project `session-handoff` entity is excluded from this trend because each Stop replaces the same row.
...
shell-safe handoff name, it offers a `memesh remember --name … --type
session-handoff --obs restart` command; otherwise it directs you to find the
